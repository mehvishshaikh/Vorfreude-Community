...
if (isset($_POST['search'])){
    ...
    $query="select * from research where description like '%$searchq%' or title like '%$searchq%'";

    $result= mysqli_query($conn,$query);

    while($row = mysqli_fetch_array($result)){
        echo "<div id='img_div'>";
            echo "<a style='text-decoration:none;' href=".$row['path']."><h4>".$row['title']."</a>";
            echo "<h2><p>Idea Description: ".$row['description']."</p></h2>";
            echo "<h3>Investment Required: ".$row['mininvestment']."-".$row['maxinvestment']."</h2>";
            echo "<h2>".$row['description']."<br><br>".$row['category']." ".$row['other_category']."</h2>";
            echo "<p>".$row['date']."</p>";
        echo"</div>";
    }
}
